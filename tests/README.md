...
| Suite | File | Focus |
|-------|------|--------|
| CLI surface | `test_cli.sh` | `sh -n`, companion digest (bare hex), `version` / `help` / `about` (human + JSON; version assertions use live `PRODUCT_VERSION`), unknown command, quiet, `CHECKSUM` not on help/about, `env -u HOME`, zero-arg install failure exit, uninstall fail-closed JSON |
